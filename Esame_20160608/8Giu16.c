	...
	for (i=0; i < N; i++)
	{
		...
		if (pid == 0) 
		{
			...
			if ((fd=open(argv[i+1], O_RDONLY)) < 0)
			{
				...
				/* OSSERVAZIONE IMPORTANTE: in realta' per come e' lo schema di comunicazione di questo compito, se un figlio muore in questa fase, il padre nella fse di invio dell'indice (calcolato in modo random) riceve il segnale SIGPIPE e quindi se il segnale non viene trattato l'azione di default e' la terminazione del padre che quindi NON attende i figli e non recupera neanche i valori di ritorno */
                       	}
			...
			/* il figlio Pi deve controllare l'ammissibilita' del valore inviato dal padre rispetto alla sua lunghezza */
				   if (r < j)
  					{
...
/* il padre calcola in modo random l'indice del figlio di cui considerera' il valore inviato */
		r=mia_random(N);
                /* stampa di controllo */
		printf("Il numero generato in modo random (per la linea %d) compreso fra 0 e %d per selezionare il figlio e' %d\n", j, N-1, r);
		for (i=0; i < N; i++)
		{ 
			/* il padre recupera tutti i valori interi dai figli */
			read(pipedFP[i][0], &valore, sizeof(valore));
			/* ma si salva il valore del figlio identificato in modo random */
			if (i == r) 
				giusto=valore;
		}	
/* il padre calcola in modo random l'indice della linea che inviera' a tutti i figli */
		r=mia_random(giusto);
                /* stampa di controllo */
		printf("Il numero generato in modo random compreso fra 0 e %d per selezionare l'indice della linea %d-esima e' %d\n", giusto-1, j, r);

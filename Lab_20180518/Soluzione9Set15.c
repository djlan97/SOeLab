...
int *confronto;			/* array di interi per sapere se si deve ancora mandare indicazione di leggere al figlio corrispondente */
pipe_t *pipeFiglioPadre;	/* array di pipe di comunicazione fra figli e padre */
pipe_t *pipePadreFiglio;	/* array di pipe di comunicazione/sincronizzazione fra padre e figlio */
int i,j; 			/* contatori */
...
/* ci vogliono almeno due file trovati che insieme con AF fa un totale di almeno 3 parametri */
if (argc < 4)
{
...
/* creazione di altre N pipe di comunicazione/sincronizzazione con il padre: N.B. invece di queste N pipe il padre potrebbe usare lo strumento dei segnali */
for (i=0;i<N;i++)
...
/* apertura file assegnato al padre */
...
for (i=0;i<N;i++){
	...
	printf("STAMPA DI DEBUGGING: Sono il figlio %d di indice %d e ho finito di leggere il file %s\n", getpid(), i, argv[i+1]); 
         /* il file e' terminato e quindi si deve ritornare al padre il valore 0 per indicare successo */
        exit(0);
	}
} /* fine for */
...
/* terminiamo forzatamente tutti i figli che hanno fallito il confronto e che sono bloccati sulla read dalla pipe con il padre */
        	if (kill(pid[i], SIGKILL) == -1) /* controlliamo che la kill non fallisca a causa della terminazione di uno dei figli, anche se in questo caso non dovremmo avere mai questo caso */
                	printf("Figlio con pid %d non esiste e quindi è già terminaro\n", pid[i]);;
    	}
 	else 
	{
